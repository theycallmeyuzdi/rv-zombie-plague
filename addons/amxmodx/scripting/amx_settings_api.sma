...
public plugin_init()
{
	register_plugin("[AMXX] Settings API", "1.1", "MeRcyLeZZ")
}
...
public native_load_setting_int(plugin_id, num_params)
{
	...
	new filename[64], setting_section[64], setting_key[64]
...
{
	formatex(linedata, len1, "%s = %.5f", setting_key, value)
}
...
FormatKeyValueArrayFloat(linedata[], len1, setting_key[], Array:array_handle)
{
	...
	// First value, append to linedata with no commas
	format(linedata, len1, "%s %.5f", linedata, ArrayGetCell(array_handle, index))
